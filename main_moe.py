...
def main(config):
    ...
    for name, param in model.named_parameters():
        if (
            ...
            and hasattr(param, "skip_allreduce")
            and param.skip_allreduce is True
        ):
            model.add_param_to_skip_allreduce(name)
            param.register_hook(partial(hook_scale_grad, dist.get_world_size()))
            logger.info(
                f"[rank{dist.get_rank()}] [{name}] skip all_reduce and div {dist.get_world_size()} for grad"
            )

    n_parameters_single = sum(
        p.numel() * model.sharded_count if hasattr(p, "skip_allreduce") else p.numel()
        for p in model.parameters()
        if p.requires_grad
    )
    logger.info(f"number of params single: {n_parameters_single}")
    n_parameters_whole = sum(
        p.numel() * model.sharded_count * model.global_experts
...
def train_one_epoch(
    ...
    for idx, (samples, targets) in enumerate(data_loader):
        ...
        loss_aux_meter.update(
            l_aux if isinstance(l_aux, float) else l_aux.item(), targets.size(0)
        )
...
def validate(config, data_loader, model):
    ...
    for idx, (images, target) in enumerate(data_loader):
        ...
        loss_aux_meter.update(
            l_aux if isinstance(l_aux, float) else l_aux.item(), target.size(0)
        )
