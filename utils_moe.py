...
def load_checkpoint(config, model, optimizer, lr_scheduler, loss_scaler, logger):
    ...
    logger.info(
        f"==============> Rank[{global_rank}] Resuming form {config.MODEL.RESUME}...................."
    )
    if config.MODEL.RESUME.endswith(f".pth"):
        ...
        logger.info(
            f"===> Rank[{global_rank}] Re-formatting checkpoint name to {resume_path}......"
        )
    else:
    ...
    if (
        ...
        logger.info(
            f"=>Rank[{global_rank}] loaded successfully '{config.MODEL.RESUME}' (epoch {checkpoint['epoch']})"
        )
...
def load_pretrained(config, model, logger):
    global_rank = dist.get_rank()
    logger.info(
        f"==============> Rank[{global_rank}] Loading weight {config.MODEL.PRETRAINED} for fine-tuning......"
    )
    if config.MODEL.PRETRAINED.endswith(f".pth"):
        ...
        logger.info(
            f"===> Rank[{global_rank}] Re-formatting checkpoint name to {pretrained_path}......"
        )
    else:
    ...
    if pretrained_path.endswith(f".rank{global_rank}"):
        ...
        if os.path.exists(pretrained_path.replace(f".rank{global_rank}", f".master")):
            checkpoint_master = torch.load(
                pretrained_path.replace(f".rank{global_rank}", f".master"),
                map_location="cpu",
            )
            state_dict = merge_moe_model_state_dict(
                checkpoint["model"], checkpoint_master["model"]
            )
        else:
        ...
        if config.TRAIN.MOE.SAVE_MASTER:
            ...
            if global_rank == 0:
                save_path = os.path.join(
                    config.OUTPUT, f"ckpt_epoch_{epoch}.pth.global"
                )
                logger.info(f"{save_path} saving......")
                torch.save(save_state, save_path)
                logger.info(f"{save_path} saved !!!")
        else:
            moe_model_state_dict, non_moe_model_state_dict = split_moe_model_state_dict(
                model._ddp_params_and_buffers_to_ignore, model.state_dict()
            )
            save_state = {"model": moe_model_state_dict}
            save_path = os.path.join(
                config.OUTPUT, f"ckpt_epoch_{epoch}.pth.rank{global_rank}"
            )
            logger.info(f"{save_path} saving......")
            torch.save(save_state, save_path)
            logger.info(f"{save_path} saved !!!")
            if global_rank == 0:
                ...
                save_path = os.path.join(
                    config.OUTPUT, f"ckpt_epoch_{epoch}.pth.master"
                )
                logger.info(f"{save_path} saving......")
...
def auto_resume_helper(output_dir, save_master=False):
    ...
    if not save_master:
        master_checkpoints = [
            ckpt for ckpt in checkpoints if ckpt.endswith(f"pth.rank0")
        ]
    else:
        master_checkpoints = [
            ckpt for ckpt in checkpoints if ckpt.endswith(f"pth.global")
        ]
    ...
    if len(master_checkpoints) > 0:
        ...
        latest_checkpoint = latest_master_checkpoint.replace(
            "pth.rank0", f"pth.rank{global_rank}"
        )
