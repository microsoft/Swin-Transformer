...
set -a options (fish_opt --short p --long nprocs --required-val --long-only)
set -a options (fish_opt --short v --long venv --required-val --long-only)
...
set launcher_args --nproc_per_node $nprocs

if not test -z $_flag_debug
    set launcher_args --nproc_per_node 1
end
